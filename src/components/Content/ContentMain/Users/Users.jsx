    ...
    return (
      ...
      <div className={s['my-listing-dt-all-follow']}>
        <div className={s['my-lisiting-picy']}>
          <a href="user_detail_view.html"> <img src={u.avatar != null ? u.avatar : userPhoto } alt="" /></a>
        </div>
        <a href="user_detail_view.html"><h4>{u.name}</h4></a>
        ...
        {u.followed ? <button className={s['folow-btn']} onClick={() => u.unfollow(u.id) }>Follow</button> : <button className={s['folow-btn']} onClick={() => u.follow(u.id) } >Unfollow</button>}
      </div>
    </div> )}
